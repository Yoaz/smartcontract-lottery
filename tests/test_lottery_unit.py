...
from scripts.helpful_scripts import get_account, get_contract, LOCAL_BLOCKCHAIN_DEVELOPMENT, fund_with_link
...
def test_can_start_and_enter_lottery():
    ...
    lottery.enter({"from": account, "value": lottery.getEntranceFee()})
    # Assert
    assert lottery.players(0) == account    # making sure the player added to the players array
...
def test_can_pick_winner():
    ...
    requestId = transaction.events["requestedRandomness"]["requestId"]   # Get the request ID from the emitted event via contract 
