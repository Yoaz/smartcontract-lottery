from brownie import accounts, Lottery, config, network, MockV3Aggregator, Contract, VRFCoordinatorMock, LinkToken
...
def get_account(index = None, id = None):
    ...
    if network.show_active() in LOCAL_BLOCKCHAIN_DEVELOPMENT or network.show_active() in FORKED_LOCAL_ENVOIRMENTAL:
        return accounts[0]
...
# @para link_token = in case of a specific link token address to use, other wise default
# @para value = default 0.1 LINK
def fund_with_link(contract_address, account = None, link_token = None, value = 100000000000000000):
